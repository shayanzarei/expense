    ...
    <section className="mx-4 mt-5">
      ...
      <div className="mt-3 rounded-xl border border-[var(--color-border)] bg-white p-3 shadow-sm">
        ...
        <div className="mb-2 flex flex-wrap items-baseline justify-between gap-1">
          ...
          <span
            className={`text-xs font-semibold ${
              lonaRemaining >= 0 ? 'text-green-600' : 'text-red-600'
            }`}
          >
            {lonaUsed > 0
              ? lonaRemaining >= 0
      ...
      <p className="mb-2 mt-3 text-xs text-[var(--color-muted)]">
        Groceries only: {formatEur(groceryMonthlyUsed)} /{' '}
        {formatEur(KHORDO_KHORAK_MONTHLY_TARGET)} ·{' '}
        <span
          className={
            groceryMonthlyRemaining >= 0 ? 'text-green-600' : 'text-red-600'
          }
        >
      ...
      <p className="text-xs text-[var(--color-muted)]">
        Combined ({formatEur(KHORDO_KHORAK_MONTHLY_TOTAL)}): {formatEur(combinedMonthlyUsed)}{' '}
        used ·{' '}
        <span
          className={
